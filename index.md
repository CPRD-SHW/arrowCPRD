...
The goal of arrowCPRD is to allow a user of CPRD data to read these
large tsv files into compressed, columnar, fast-reading
[parquet](https://parquet.apache.org/docs/) files. You can read your
data in after unzipping it, or read it straight from the zipped files!
To get started, try some things below.
...
You can do this for each of the tables (‘patient’, ‘observation’,
‘practice’) in your dataset.

`pq_in` then functions like a normal data frame (running `collect` to
bring into R session after computations):
...
If you’ve unzipped your CPRD data, you can use `read_files_from_tsv` on
the whole folder also.
